...
function drawSkeletonPoints(input, hms, coords, max)
    ...
    local occlusion_mask = max:le(occlusion_th):reshape(16,1)

    local actThresh = 0.002
    local green = {0,255,0}
    local blue = {0,0,255}
    local tmp_x = coords:select(2,1)[occlusion_mask]
    local tmp_y = coords:select(2,2)[occlusion_mask]
    local tmp_ind_name = torch.range(1,#partNames)[occlusion_mask]
    
    if(occlusion_mask:sum() > 0) then
        print('------------- OCCLUDED -------------')
        local occluded_points = torch.Tensor(tmp_x:size(1),2)
        occluded_points:select(2,1):add(tmp_x)
        occluded_points:select(2,2):add(tmp_y)
        for i=1, occluded_points:size(1) do
            if hms[i]:mean() > actThresh then        
                print('part: ', partNames[tmp_ind_name[i]])
                drawPoint(im, occluded_points[i], 3, blue)
            end
        end
    end
    
    visible_mask = occlusion_mask:eq(0)
    tmp_x = coords:select(2,1)[visible_mask]
    tmp_y = coords:select(2,2)[visible_mask]
    tmp_ind_name = torch.range(1, #partNames)[visible_mask]
    
    if(visible_mask:sum() > 0) then

        local visible_points = torch.Tensor(tmp_x:size(1),2)
        
        visible_points:select(2,1):add(tmp_x)
        visible_points:select(2,2):add(tmp_y)

        print('------------- VISIBLE -------------')
        for i=1, visible_points:size(1) do        
            if hms[i]:mean() > actThresh then
                print('part: ', partNames[tmp_ind_name[i]])
                drawPoint(im, visible_points[i], 3,green)
            end
        end
    end
...
function drawSkeleton(input, hms, coords)
    ...
    for i = 1,#pairRef do
        if hms[pairRef[i][1]]:mean() > actThresh and hms[pairRef[i][2]]:mean() > actThresh then
            ...
            -- Draw line
            --drawPoint(im, coords[pairRef[i][1]], 3, green)
            --drawPoint(im, coords[pairRef[i][2]], 3, green)
...
function drawOutput(input, hms, coords, max)
    --local im = drawSkeleton(input, hms, coords)
    local im = drawSkeletonPoints(input, hms, coords, max)
    local colorHms = {}
