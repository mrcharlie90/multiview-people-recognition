...
function drawPoint(img,point,width,color1)
    -- I'm sure there's a line drawing function somewhere in Torch,
    -- but since I couldn't find it here's my basic implementation
    local x = point[1]
    local y = point[2]
    if((x + width) < img:size(3) and (y + width) < img:size(2) and x > 0 and y > 0) then
        image.drawRect(img, x, y, x + width, y + width, {lineWidth = 5, color = color1, inplace = true})
    end
    return img
end
